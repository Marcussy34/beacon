...
export function resolvePidTty(pid: number): Promise<string | null> {
  return new Promise((resolve) => {
    // execFile/spawn can throw synchronously on spawn failures (e.g. EPERM/ENOENT).
    // Catch and resolve(null) so the promise never rejects.
    try {
      ...
      // Synchronous spawn failure — treat as unresolvable.
      resolve(null);
    }
  });
}
